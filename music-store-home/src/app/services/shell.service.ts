...
export class ShellService {
  ...
  urlChanged() {
    for (const client in this.config.clients) {
      ...
      if (location.pathname.startsWith(entry.route)) {
  ...
  load(name: string): void {
    ...
    // Add tag for micro frontend, e. g. <ms-albums></ms-albums>
    const element = document.createElement(configItem.element);
    element['hidden'] = !location.pathname.startsWith(configItem.route);
  ...
  navigate(url: string) {
    const pos = location.pathname.indexOf('?');
    const query = pos !== -1? location.pathname.substr(pos): '';
